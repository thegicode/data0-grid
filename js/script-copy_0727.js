...
function copyCells() {
    ...
    copyedCells = sortedCells.map((cell) => cell.cloneNode(true));
    return copyedCells;
}
...
function pasteCells(clipboardText) {
    ...
    // copyedCells.forEach((cell, index) => {
    //     const input = cell.querySelector("input");
    //     if (index === 0) {
    //         firstSelectedCell.replaceWith(cell);
    //         firstSelectedCell.querySelector("input").dataset.row = targetRow;
    //         firstSelectedCell.querySelector("input").dataset.col = targetCol;
    //     }

    //     if (index == 1) {
    //         if (
    //             input.dataset.row ===
    //             copyedCells[index - 1].querySelector("input").dataset.row
    //         ) {
    //         }
    //     }
    // });
...
document.addEventListener("copy", (e) => {
    const elements = copyCells();
    // console.log("copy elements", elements);

    const clipboardData = elements.map((el) => el.outerHTML).join("\n");
    // console.log("copy clipboardData", clipboardData);
...
document.addEventListener("paste", (e) => {
    ...
    // console.log("paste clipboardText", clipboardText);
...
grid.addEventListener("dragstart", (e) => {
    ...
    if (cell && selectedCells.has(cell)) {
        const clipboardData = selectedCells
            .map((cell) => cell.cloneNode(true))
            .map((el) => el.outerHTML)
            .join("\n");
        e.dataTransfer.setData("text/plain", clipboardData);
    } else {
        e.preventDefault();
    }
});
